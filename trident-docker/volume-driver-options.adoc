...
== Cloud Volumes Service (CVS) auf AWS Volume-Optionen

Zu den Optionen zur Erstellung von Volumes für den CVS auf AWS-Treiber gehören:
...
| `snapshotReserve`  a| 
`Hiermit wird die Snapshot-Reserve auf den gewünschten Prozentsatz eingestellt. Der Standardwert ist kein Wert, was bedeutet, dass CVS die Snapshot-Reserve wählt (normalerweise 0%).
...
----
* 500 gib Premium-Volume erstellen:
+
[listing]
----
